...
// sunburst, optionally on clouds, with transparent background
export default function vizSunburst (options = {}) {
  ...
  const draw = function (spectrum) {
    ...
    // --- サークルマスク ---
    ctx.beginPath()
    ctx.arc(cv.width / 2, cv.height / 2, Math.min(cv.width, cv.height) / 2, 0, Math.PI * 2)
    ctx.clip()
    ...
    if (clouds) {
      ...
      for (let i = 0; i < particles.length; i++) {
        ...
        ctx.drawImage(particleImage, particles[i].x, particles[i].y)
        particles[i].move()
      }
    }
