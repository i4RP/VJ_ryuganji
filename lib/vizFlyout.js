...
export default function vizBoxes(options = {}) {
  ...
  const metallicPalette = ['#d4af37', '#c0c0c0', '#b08d57', '#e6e8fa', '#f5f5f5', '#f0e68c']
  ...
  function draw(spectrum) {
    ...
    if (rotationSpeedTarget < desiredAutoSpeed) {
      rotationSpeedTarget += (desiredAutoSpeed - rotationSpeedTarget) * 0.05 // テンポ上昇追従
    } else if (rotationSpeedTarget > 1) {
      rotationSpeedTarget += (1 - rotationSpeedTarget) * 0.005 // ゆっくり戻る
    }
    ...
    for (let row = 0; row < gridRows; row++) {
      for (let col = 0; col < gridCols; col++) {
        ...
        const intensity = grow ? (spectrum[index] || 0) / 255 / 4 + 1.25 : 0.5
        let size = step * intensity
        let drawX = rotX
        let drawY = rotY
        ...
        if (isKaleido) {
          ...
          size *= 1.5 + Math.sin(norm * 10 + now / 350) * 0.5  // 花びらの開閉感
